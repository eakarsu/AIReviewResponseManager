// AI Personalized review response generation
// AI learns brand voice, generates contextual responses
...
async function callLLM(systemPrompt, userPrompt) {
  ...
  const response = await fetch(baseUrl + '/chat/completions', {
    ...
    headers: {
      ...
      'X-Title': 'AIReviewResponseManager'
    },
    body: JSON.stringify({
      model: MODEL,
      messages: [
...
router.post('/', async (req, res) => {
  try {
    ...
    const systemPrompt = `You are an expert AI assistant for AIReviewResponseManager. Focus area: Personalized review response generation. ${`AI learns brand voice, generates contextual responses`}. Respond ONLY with valid JSON (no markdown fences).`;
    const userPrompt = `Task: Personalized review response generation.\n${`AI learns brand voice, generates contextual responses`}\n\nInput payload (JSON):\n${JSON.stringify(context, null, 2)}\n\nReturn JSON with the shape:\n{\n  "summary": "...",\n  "findings": ["..."],\n  "recommendations": ["..."],\n  "score": 0,\n  "confidence": 0\n}`;
    ...
    await persistResult(req.user?.id, 'generate-response', context, parsed);
    res.json({ feature: 'generate-response', model: MODEL, result: parsed });
  } catch (err) {
    console.error('[generate-response]', err.message);
...
router.get('/history', async (req, res) => {
  try {
    const r = await pool.query('SELECT id, endpoint, input_data, result, created_at FROM ai_results WHERE endpoint=$1 AND user_id=$2 ORDER BY created_at DESC LIMIT 50', ['generate-response', req.user.id]);
